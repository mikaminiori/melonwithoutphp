...
interface UnderscoreStatic {
	...
	**/
    ...
    /**
    * Caution : Developper Added this!
	* Looks through each value in the list, returning the index of the last one that passes a truth
	* test (iterator). The function returns as soon as it finds an acceptable element,
	* and doesn't traverse the entire list.
	* @param list Searches for a value in this list.
	* @param iterator Search iterator function for each element in `list`.
	* @param context `this` object in `iterator`, optional.
	* @return The index of the last acceptable found element in `list`, if nothing is found -1 is returned.
	**/
    findLastIndex<T>(
        list: _.List<T>,
        iterator: _.ListIterator<T, boolean>,
